...
当前代码中，Quality Layer 的第一阶段已经通过现有 AuditEvent 体系落地在 Owner Schema Governance 场景中。

已实现：

1. `AuditEventRecord` / `AuditEventType` / `AuditActor` / `AuditTarget` / `AuditEventPayload`；
2. `AppendAuditEventRequest` / `QueryAuditEventsResponse`；
3. mock append-only audit store；
4. `idempotencyKey` 去重；
5. Owner schema publish audit events；
6. Owner Audit Timeline；
7. Schema Governance demo data。

未实现，属于未来规划：

1. 显式 `QualityLedgerDomain` 字段；
2. `DataQualityPassport`；
3. Labeler telemetry；
4. Reviewer diff audit；
5. Prompt Feedback Loop；
6. Risk Fast Path / Quality Slow Path；
7. Redis risk state；
8. Hot / Warm / Cold lifecycle；
9. Read Model / Snapshot；
10. `CLIENT_TIME_DRIFT`；
11. `sendBeacon` fallback；
12. IndexedDB sync queue。
...
目标态中，所有质量事件进入同一个 Ledger，通过 `domain` 字段区分来源域。

当前第一阶段尚未在 contracts 中实现显式 `domain` 字段。Owner Schema Governance audit events 可以视为 `SCHEMA_GOVERNANCE` domain 的第一阶段实现，但该 domain 目前是逻辑域，不是物理字段。

当前实现可通过以下字段推导事件域：

```txt
AuditEventRecord.type
AuditEventRecord.target.entityType
AuditEventRecord.source
```

如果未来确实需要显式 domain，应在现有 `AuditEventRecord` 中最小新增 `domain?: AuditEventDomain`，不要新增一套平行的 Ledger event 类型。
...
### 5.1 QualityLedgerEvent（概念模型，不是当前 contracts 新增类型）

本节中的 `QualityLedgerEvent` 是目标架构概念模型，不是要求新增的 contracts 类型。

当前实现应优先复用现有 `AuditEventRecord`。如果未来确实需要补充 Quality Ledger 字段，应最小扩展 `AuditEventRecord`，而不是新增一套平行的 `QualityLedgerEvent`。
...
| 概念字段 | 当前实现 |
|---|---|
| `QualityLedgerEvent.eventId` | `AuditEventRecord.id` |
| `QualityLedgerEvent.eventType` | `AuditEventRecord.type` |
| `QualityLedgerEvent.occurredAt` | `AuditEventRecord.createdAt` |
| `QualityLedgerEvent.payload` | `AuditEventRecord.payload` |
| `QualityLedgerEvent.userId` | `AuditEventRecord.actor.id` |
| `taskId` / `submissionId` / `reviewId` / `exportId` | 优先放在 `AuditEventRecord.target`，必要摘要放在 `payload` |

### 5.2 eventId 与 idempotencyKey 的区别

`eventId` 和 `idempotencyKey` 不能混用。

当前代码中，事件记录 ID 使用 `AuditEventRecord.id`。文档中的 `eventId` 概念对应 `AuditEventRecord.id`，不要求新增 `eventId` 字段。
...
普通 telemetry 事件未来可以使用 `sessionToken` 组成幂等键；核心提交、审核、导出、发布事件应优先绑定业务 ID 与内容 hash。当前 Owner Schema Governance 第一阶段已经在关键发布事件中使用 `idempotencyKey`，Labeler / Reviewer / Export 仍属于未来接入范围。
...
当前实现中，服务端可信事件时间使用 `AuditEventRecord.createdAt`。文档中的 `occurredAt` 是概念命名，对应当前 `createdAt`，第一阶段不新增 `occurredAt` 字段。

如果未来需要客户端行为时间，可以新增 `clientOccurredAt`，或先放入具体 payload 中。不要让 `createdAt` 与 `occurredAt` 同时承担两套服务端可信时间语义。
...
3. 不阻塞主业务流程；
4. 支持幂等写入；
5. 目标态支持按 domain / taskId / schemaVersionId / submissionId / userId / eventType 查询；
6. 后续可异步聚合为质量指标、风控状态和 Data Quality Passport。

当前第一阶段尚无物理 `domain` 字段，查询能力应优先复用 `AuditEventRecord.type`、`target` 和 `source`。
...
1. `DataQualityPassport` 尚未在 contracts 中实现；
2. 当前 Export contracts 已有 `ExportMapping`、`ExportWarning`、`ExportRecordMetadata` 等基础类型；
3. 未来不应让 `DataQualityPassport` 与 `ExportRecordMetadata` 职责重叠；
4. 建议将 Passport 作为 `ExportRecord` / export artifact 的一部分进行扩展；
5. Quality Ledger 第一阶段只记录 Owner Schema Governance 事件，不生成 Passport。
...
1. `packages/schema-core` 已实现 `stableStringify`，可作为 `canonical-json-v1` 的基础；
2. `packages/contracts` 已有 `CanonicalSerializationVersion` / `ChecksumInputEnvelope`；
3. 当前尚未实现全链路权威 SHA-256 生成；
4. 未来 `answerHash` / `outputHash` / `promptSnapshotHash` / `passportBatchHash` 应复用 `stableStringify` / `canonical-json-v1` 规则；
5. 真实 SHA-256 应由后端或统一 shared utility 生成；
6. 不要在前端和后端各自手写不一致的 hash 逻辑。
...
1. 当前 Labeler 页面已有 assignment lifecycle、answers 和提交入口；
2. `LABELING_SESSION_SUMMARY`、`FORM_ABANDONED`、`CLIENT_TIME_DRIFT` 尚未实现；
3. `sendBeacon` fallback 和 IndexedDB sync queue 尚未实现；
4. 后续应先扩展 contracts 中的 telemetry event type 和 payload 类型，再接入 Labeler 页面 session timer；
5. 不要先在页面中定义局部 telemetry 类型。
...
1. 当前 Reviewer 页面已有审核详情和提交入口；
2. 结构化 `REVIEW_DIFF_GENERATED` 尚未实现；
3. `beforeAnswerHash`、`afterAnswerHash`、`patchedFieldNames`、`diffMode` 尚未作为 contracts 事件 payload 落地；
4. Reviewer diff 应排在 Labeler telemetry 之后；
5. 第一阶段可先做 shallow diff；
6. 不要在 Reviewer 页面中私自定义局部 diff 类型，应先补 contracts。
...
当前第一阶段尚未在 `AuditEventRecord` 中实现显式 `domain` 字段。Owner Schema Governance audit events 已可视为 `SCHEMA_GOVERNANCE` 逻辑域，当前通过 `AuditEventRecord.type`、`target.entityType` 和 `source` 推导。
...
当前 `stableStringify` 已实现 canonical string 规则；真实 SHA-256 尚未实现。后续 answer / output / prompt / passport hash 应复用同一 canonical serialization 规则，并通过 test vectors 保证前后端一致。
...
### QL-0A：文档与代码真实性对齐审查（已完成）

```txt
只读审查 Quality Layer 文档与当前真实代码的匹配度。
```

### QL-0B：文档对齐当前代码（本轮）

```txt
保留目标架构，同时明确当前已实现能力、未来规划边界和 contracts 复用规则。
```

### QL-1：contracts 最小扩展 Labeler telemetry / Reviewer diff / AI feedback 所需事件类型

```txt
在 @labelhub/contracts 中补齐 Labeler telemetry、Reviewer diff、AI feedback 相关事件类型和 payload 类型。
```

注意：

```txt
不要先在 apps/web 页面中定义局部 telemetry / diff 类型。
```

### QL-2：mock / API 扩展 telemetry append / query 能力

```txt
在 mock / API 层扩展必要的 telemetry append / query 能力。
```

### QL-3：Labeler telemetry 最小接入

```txt
接入 session timer、提交摘要、FORM_ABANDONED 和必要的 riskSignals。
```

### QL-4：Reviewer diff event 最小接入

```txt
接入 REVIEW_DIFF_GENERATED / REVIEW_SUBMITTED 的浅层 diff 摘要。
```

### QL-5：Export / Data Quality Passport contracts 与 mock

```txt
补齐 DataQualityPassport 与 export artifact metadata 的 contracts / mock 基础。
```

### QL-6：Prompt Feedback Loop summary / hash 接入

```txt
记录 AI Assist / AI Review 的 summary、hash、promptVersionId 和采纳结果，不保存 raw prompt / raw output。
```

### QL-7：Read Model / Snapshot / risk state 规划实现

```txt
在基础事件稳定后，再实现 read model、snapshot、risk fast path 和生命周期管理。
```
...
Redis risk state
Quality Metrics Read Model
Data Quality Passport 批量生成
```
...
系统把标注行为、AI 辅助、人工审核、Schema 发布、数据迁移和导出统一建模为质量事件，进入同一个 Quality Ledger。目标态中不同业务域通过 domain 字段区分；当前第一阶段通过 `AuditEventRecord.type`、`target.entityType` 和 `source` 推导业务域，让一条数据从 Schema 设计到最终导出的完整链路可以被还原。
...
本节描述 Quality Layer 的目标态验收标准。当前第一阶段已实现 Owner Schema Governance audit 子集，尚未实现显式 `domain`、`DataQualityPassport`、Labeler telemetry、Reviewer diff、Prompt Feedback Loop 和工业化存储生命周期。

### 21.1 功能标准

- Quality Ledger 支持 append-only 写入，不允许更新和删除原始事件。
- 幂等写入基于 idempotencyKey 去重，相同 key 不重复写入。
- 目标态中所有域事件进入同一 Ledger，通过 domain 区分；当前第一阶段可通过 `AuditEventRecord.type` / `target.entityType` / `source` 推导。
- Migration 事件归入 SCHEMA_GOVERNANCE 逻辑 domain，不单独设 MIGRATION domain。
- 文档中的 `eventId` 对应当前 `AuditEventRecord.id`，`idempotencyKey` 是业务幂等键。
- 文档中的 `occurredAt` 对应当前 `AuditEventRecord.createdAt`；如未来新增 `clientOccurredAt`，它只用于行为分析。
...
5. `AuditEventRecord.id` 和 `idempotencyKey` 的区别是什么？
6. 为什么客户端时间不可信？
