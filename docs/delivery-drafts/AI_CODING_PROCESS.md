...
> **状态：草稿。** 本文件目前位于 `docs/delivery-drafts/` 下，因为最终提交目录尚未固定，仓库仍在整理中。本文档可以移动，并且不包含任何假设最终路径的已提交引用。迁移建议见第 7 节。
...
本草稿特意保持路径无关。当最终提交目录确定后，可以考虑放在：

* `submission/process/AI_CODING_PROCESS.md`，与提交 README 放在一起；或
* 保留在 `docs/` 下，作为仓库内记录：`docs/AI_CODING_PROCESS.md`。

在此之前，它暂时保留在 `docs/delivery-drafts/` 下，不作为任何必需交付路径的一部分。
