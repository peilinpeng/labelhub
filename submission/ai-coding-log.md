...
- **「半个改动」比不改更危险**：收尾阶段「打回意见回传」功能，一次提交（`f30af86`）让后端 `get_assignment_context` 在 `RETURNED` 状态返回序列化后的 `ReviewResult` dict，却漏改了响应模型字段 `lastReturnReason: str | None`——类型层与序列化层不一致，导致该接口在打回态直接 500，恰好砸掉「标注员看打回意见→修改重提」主线。通过复跑类型校验/pytest 定位，补上 `dict | None`（`e052a25`）才闭环。说明 AI/自动化产出的「结构正确但端到端类型不一致」最隐蔽，必须人工跑一遍真实链路验收。
